...
/** clave Ãºnica por combinaciÃ³n de tamaÃ±os + preferred + usdPerTon (ordenada) */
...
/** Agrupa gramajes de un mismo material cuando tienen exactamente los mismos tamaÃ±os y precio. */
export function groupEquivalentGrams(items: MatType[]): MatType[] {
  return items.map(item => {
    ...
    const grams: MatGram[] = Object.keys(bySignature).map(sig => {
      const nums = Array.from(new Set(bySignature[sig])).sort((a,b)=>a-b); // Ãºnicos y ordenados
      // mantenemos gram como number|number[] (si hay uno solo, nÃºmero; si no, array)
